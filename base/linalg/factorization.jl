...
function (\){T<:BlasFloat}(A::QRPivoted{T}, B::StridedMatrix{T}, rcond::Real)
    r = A[:R]
    nr = size(r, 1)
    if abs(r[1]) < rcond error("Left hand side is zero") end
    rnk = 1
    xmin = ones(T, nr)
    xmax = ones(T, nr)
    tmin = abs(r[1])
    tmax = abs(r[1])
    while rnk < nr
        tmin, smin, cmin = LAPACK.laic1!(2, sub(xmin, 1:rnk), tmin, sub(r, 1:rnk, rnk + 1), r[rnk + 1, rnk + 1])
        tmax, smax, cmax = LAPACK.laic1!(1, sub(xmax, 1:rnk), tmax, sub(r, 1:rnk, rnk + 1), r[rnk + 1, rnk + 1])
