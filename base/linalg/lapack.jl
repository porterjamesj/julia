...
for (tzrzf, ormrz, elty) in 
 ...
 #   27 *       COMPLEX*16         A( LDA, * ), TAU( * ), WORK( * )
        function tzrzf!(A::StridedMatrix{$elty})
            m, n = size(A)
            if n < m throw(DimensionMismatch("Matrix cannot have less columns than rows")) end
            lda = max(1, m)
            tau = Array($elty, m)
            work = Array($elty, 1)
            lwork = -1
...
for (laic1, elty) in
   ...
   # 28 *       DOUBLE PRECISION   W( J ), X( J )
        function laic1!(job::BlasInt, x::StridedVector{$elty}, sest::$elty, w::StridedVector{$elty}, gamma::$elty)
            j = length(x)
